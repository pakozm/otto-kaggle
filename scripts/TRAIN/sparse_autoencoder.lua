...
local HSIZE        = tonumber(arg[1] or 900)
local bunch_size   = tonumber(arg[2] or 512)
local wd           = tonumber(arg[3] or 0.0)
local var          = tonumber(arg[4] or 0.0)
local mp           = tonumber(arg[5] or 999)
local feats_name   = arg[6] or "int600"
local opt          = arg[7] or "adadelta"
...
local model = ann.components.stack():
  push(
    ...
    actf.sparse_logistic{ sparsity=0.05, penalty=1 },
...
write_code(train_data, "DATA/train_feats.sae%d.split.mat.gz"%{ HSIZE })
write_code(val_data, "DATA/val_feats.sae%d.split.mat.gz"%{ HSIZE })
...
write_code(test_data, "DATA/test_feats.sae%d.split.mat.gz"%{ HSIZE })
